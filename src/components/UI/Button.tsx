...
export default function Button({
    ...
    const variants = {
        primary:
            'bg-acid text-void hover:bg-white hover:text-void border border-acid',
        secondary:
            'bg-transparent text-stark border border-stark hover:bg-stark hover:text-void',
    ...
    return (
        <button
            ...
            {...props}
        >
            {Icon && <Icon className={`w-4 h-4 ${hasChildren && 'mr-2'}`} />}
            {children}
        </button>
    )
}
